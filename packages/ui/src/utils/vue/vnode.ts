...
/**
 * определить, является ли элемент допустимым типом элемента, а не фрагментами и комментарием, например  <template> v-if
 */
...
export const getNormalizedProps = (node: VNode) => {
  if (!isVNode(node)) {
    debugWarn(SCOPE, "[getNormalizedProps] должен быть VNode");
    return {};
  }
...
export const ensureOnlyChild = (children: VNodeArrayChildren | undefined) => {
  if (!isArray(children) || children.length > 1) {
    throw new Error("ожидать получения одного дочернего элемента Vue");
  }
  return children[0];
};
